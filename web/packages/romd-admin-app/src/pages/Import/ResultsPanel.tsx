...
export function ResultsPanel({ job, onDismiss }: { job: JobDtoUploadJobDto; onDismiss: () => void }) {
  ...
  const issues = job.hasErrors || (!job.trackedOnly && Number(job.romsRejected) > 0);
  ...
  return <section><Stack gap="md">
    <Group justify="space-between" align="flex-start"><Stack gap="xs" style={{ minWidth: 0, flex: '1 1 300px' }}>
      ...
      <Group gap="md"><Text size="sm">{Number(job.romsIngested).toLocaleString()} stored</Text><Text size="sm" c="dimmed">{Number(job.romsDeduplicated).toLocaleString()} already stored</Text>
        {!!Number(job.romsRejected) && <Text size="sm" c="orange">{Number(job.romsRejected).toLocaleString()} {job.trackedOnly ? 'skipped' : 'not stored'}</Text>}
    ...
    {!!Number(job.romsRejected) && (job.trackedOnly
      ? <Alert color="blue" title="Tracked-title filter applied">ROMs without a match to a currently tracked title were skipped. Track additional titles and upload again to keep them.</Alert>
      : <Alert color="orange" title="Unmatched files were not stored">Import a matching DAT and upload those files again, or enable Keep unmatched files on your next import.</Alert>)}
